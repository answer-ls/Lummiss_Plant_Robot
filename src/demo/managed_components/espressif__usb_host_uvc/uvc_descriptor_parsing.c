...
/* 新摄像头对照测试：固定选择 alt=1，使有效 MPS=3072 与
 * UVC Probe/Commit 协商出的最大负载保持一致。改为 0 可恢复自动选择。 */
#define UVC_TEST_FORCE_ISOC_ALT 1
    ...
    const usb_ep_desc_t **ep_desc_ret)
{
    ...
    bool selected_is_bulk = false;
    unsigned bulk_candidates = 0;
    unsigned isoc_candidates = 0;
    for (int i = 0; i <= num_of_alternate; i++) {
        ...
        const uint8_t transfer_type = USB_EP_DESC_GET_XFERTYPE(ep_desc);
        const bool current_is_bulk = transfer_type == USB_BM_ATTRIBUTES_XFER_BULK;
        const bool current_is_isoc = transfer_type == USB_BM_ATTRIBUTES_XFER_ISOC;

        /* Print every alternate setting so endpoint selection can be verified
         * against the camera's negotiated dwMaxPayloadTransferSize.  The raw
         * wMaxPacketSize is not the actual HS isochronous capacity: MULT adds
         * additional transactions in the same 125 us microframe. */
        if (current_is_isoc) {
            ESP_LOGI(TAG,
                     "VS接口=%u ISOC候选 alt=%u addr=0x%02x raw_MPS=%u MULT=%u "
                     "effective_MPS=%u request_max=%u %s",
                     bInterfaceNumber, intf_desc->bAlternateSetting,
                     ep_desc->bEndpointAddress, USB_EP_DESC_GET_MPS(ep_desc),
                     packets_per_frame, current_mps, dwMaxPayloadTransferSize,
                     current_mps <= dwMaxPayloadTransferSize ? "可选" : "超过请求上限");
        } else if (current_is_bulk) {
            ESP_LOGI(TAG,
                     "VS接口=%u BULK候选 alt=%u addr=0x%02x raw_MPS=%u request_max=%u",
                     bInterfaceNumber, intf_desc->bAlternateSetting,
                     ep_desc->bEndpointAddress, USB_EP_DESC_GET_MPS(ep_desc),
         ...
         * largest such endpoint and ignore larger ISOC alternates.  Cameras
         * that only expose ISOC retain the original largest-MPS selection. */
        const bool better_bulk = current_is_bulk &&
                                 (!selected_is_bulk || current_mps > last_mps);
        const bool better_isoc = current_is_isoc && !selected_is_bulk &&
                                 current_mps >= last_mps;
        if (current_mps <= dwMaxPayloadTransferSize && (better_bulk || better_isoc)) {
            last_mps = current_mps;
            selected_is_bulk = current_is_bulk;
            *ep_desc_ret = ep_desc;
    ...
    if (ret == ESP_OK) {
        const uint8_t selected_type = USB_EP_DESC_GET_XFERTYPE(*ep_desc_ret);
        const uint8_t selected_packets = USB_EP_DESC_GET_MULT(*ep_desc_ret) + 1;
        const uint16_t selected_effective_mps =
            USB_EP_DESC_GET_MPS(*ep_desc_ret) * selected_packets;
        ESP_LOGI(TAG,
                 "VS接口=%u 端点能力：BULK=%u、ISOC=%u；选择=%s alt=%u addr=0x%02x "
                 "raw_MPS=%u MULT=%u effective_MPS=%u request_max=%u",
                 bInterfaceNumber, bulk_candidates, isoc_candidates,
                 selected_type == USB_BM_ATTRIBUTES_XFER_BULK ? "BULK" :
                 (selected_type == USB_BM_ATTRIBUTES_XFER_ISOC ? "ISOC" : "OTHER"),
                 (*intf_desc_ret)->bAlternateSetting,
                 (*ep_desc_ret)->bEndpointAddress,
                 USB_EP_DESC_GET_MPS(*ep_desc_ret), selected_packets,
                 selected_effective_mps, dwMaxPayloadTransferSize);
    } else {
        ESP_LOGW(TAG, "VS接口=%u 未找到可用视频端点：BULK=%u、ISOC=%u、最大请求=%u",
                 bInterfaceNumber, bulk_candidates, isoc_candidates,
                 dwMaxPayloadTransferSize);
    }
